...
namespace commodity\modules\pickingdestroy\models\put;

use common\models\Model as CommonModel;
use commodity\modules\pickingdestroy\models\put\db\Insert;
use commodity\modules\pickingdestroy\models\put\db\Get;
use Yii;
...
//            print_r($add_data);die;
    ...
    public static function prepareData(array $data, $switch = true) {
        ...
        $pickingdestroy_data['type'] = $type = array_key_exists('type', $data) ? $data['type'] : '';
        
        $pickingdestroy_data['commodity_id'] = $condition['commodity_id']=array_key_exists('commodity_id', $data) ? $data['commodity_id'] : '';

        $pickingdestroy_data['count']=$count=array_key_exists('count', $data) ? $data['count'] : '';


        $pickingdestroy_data['status'] = array_key_exists('status', $data) ? $data['status'] : '';

        $pickingdestroy_data['store_id'] = $condition['store_id'] = current($userIdentity)['store_id'];
        ...
        $pickingdestroy_data['last_modified_by'] = current($userIdentity)['id'];
        $pickingdestroy_data['last_modified_time'] = date('Y-m-d H:i:s');

        if ($switch) {
            $pickingdestroy_data['created_time'] = date('Y-m-d H:i:s');
            $pickingdestroy_data['created_by'] = current($userIdentity)['id'];
        }


        return array_filter($pickingdestroy_data, function ($v) {
