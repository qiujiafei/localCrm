...
use commodity\modules\member\models\cardTemplate\templates\DiscountTemplate;
use common\components\tokenAuthentication\AccessTokenAuthentication;
use Yii;

class DiscountTemplateModel extends CommonModel
{
    ...
    public function rules() {
        return [
            [
                ['id'], 'required', 'message' => 22001, 'on'=>[self::ACTION_GET_ONE, self::ACTION_SET_ONE, self::ACTION_PUT_BATCH, self::ACTION_GET_BATCH]
            ],
            [
            ...
            ['name', 'string', 'message' => 22003],
            ['recharge_money', 'give_money'], 'required', 'message' => 22001, 'on'=>self::ACTION_PUT_ONE,
            ['recharge_money', 'string', 'length'=>[1,12], 'message' => 22004],
            ['comment', 'string', 'length'=>[0,30], 'message' => 22005],
    ...
    public function actionGetOne() {
        try {
            ...
            $template->loadTemplate(new DiscountTemplate());
            $result = $template->getOne($this->id);
            if (!$result) {
                $this->addError('DiscountTemplateModel/getone',21006);
            }
        } catch (\Exception $e) {
            $this->addError('DiscountTemplateModel/getone', $e->getCode());
            return false;
        }
        return $result;
    }
    ...
    public function actionGetAll() {
        try {
            $column = ['id','name','recharge_money','comment'];
            $condition = ['and'];
            ...
            $this->addError('DiscountTemplateModel/getall', $e->getCode());
            return false;
        }
        return [
            'count' => $result->count,
    ...
    public function actionPutOne() {
        try {
            ...
            $this->addError('DiscountTemplateModel/putone', $e->getCode());
            return false;
        }
        return true;
    }
    ...
    public function actionPutBatch() {
        try {
            ...
            $this->addError('DiscountTemplateModel/putbatch', $e->getCode());
            return false;
        }
        return true;
    }
