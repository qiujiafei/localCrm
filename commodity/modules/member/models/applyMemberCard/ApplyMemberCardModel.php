 ...
 *
 * @author: GFi <qiujiafei@9daye.com.cn>
 */
...
use commodity\modules\member\models\memberCardType\get\SelectType;
...
class ApplyMemberCardModel extends CommonModel {
    ...
    public function actionApplymembercard() {
        ...
        $cardTypeAll = SelectType::getCardTypeAll();
        $cardTypeAll = array_column($cardTypeAll,'type');
        $memberCardTypeId = $this->member_card_type_id;

        try{
            if(!in_array($memberCardTypeId,$cardTypeAll)){
        ...
        if($result['status'] == 1){
            ...
            if($data['member_card_type_id'] == 2){
                ...
                $serviceDetails = $template->getService($data['member_template_id']);
    ...
    public static function getMemberNumber($len = 8, array $data){
        ...
        $acronym = SelectType::getMemberCardAcronym($data['member_card_type_id']);
