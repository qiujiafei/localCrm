...
namespace commodity\modules\pickingcommodity\models\modify;

use Yii;
use common\models\Model as CommonModel;
use commodity\modules\pickingcommodity\models\modify\db\Update;
...
class ModifyModel extends CommonModel {
    ...
    public $token;
    public $name; //姓名 
    public $pickingcommodity_number; //工号 
    public $pickingcommodity_type_name; //工种名 
    ...
    public function scenarios() {
        return [
            self::ACTION_MODIFY => [
                'name', 'phone_number', 'pickingcommodity_number', 'pickingcommodity_type_name', 'qq_number', 'basic_salary', 'ability', 'attendance_code', 'ID_card_image', 'ID_code', 'comment', 'token'
            ],
            self::ACTION_LEAVE => [
    ...
    public function actionModify() {
        ...
        try {
            ...
            $post_data['pickingcommodity_number'] = $this->pickingcommodity_number;
            $post_data['name'] = $this->name;
            $post_data['pickingcommodity_type_name'] = $this->pickingcommodity_type_name;
            ...
            //整理参数
            $modify_pickingcommodity_data = PutModel::prepareData($post_data, false);



            $condition['store_id'] = $modify_pickingcommodity_data['store_id'];
            $condition['pickingcommodity_number'] = $this->pickingcommodity_number;
            unset($modify_pickingcommodity_data['pickingcommodity_number']);
//            print_r($condition);
//            die;
            //更改操作
            if (!Update::modifyEmployee($condition, $modify_pickingcommodity_data)) {
    ...
    public function actionLeave() {
        ...
        try {
            ...
            $modify_pickingcommodity_data['status'] = 1;
            //更改操作
            if (Update::modifyAllEmployee($condition, $modify_pickingcommodity_data) === false) {
    ...
    public function actionOpen() {
        ...
        try {
            ...
            $modify_pickingcommodity_data['status'] = 0;
            //更改操作
            if (Update::modifyAllEmployee($condition, $modify_pickingcommodity_data) === false) {
