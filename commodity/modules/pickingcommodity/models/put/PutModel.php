...
namespace commodity\modules\pickingcommodity\models\put;

use common\models\Model as CommonModel;
use commodity\modules\pickingcommodity\models\put\db\Insert;
use commodity\modules\pickingcommodity\models\put\db\Get;
use Yii;
...
//            print_r($add_data);die;
    ...
    public static function prepareData(array $data, $switch = true) {
        ...
        //生成pickingcommodity参数


        $pickingcommodity_data['type'] = $type = array_key_exists('type', $data) ? $data['type'] : '';
        
        $pickingcommodity_data['commodity_id'] = $condition['commodity_id']=array_key_exists('commodity_id', $data) ? $data['commodity_id'] : '';

        $pickingcommodity_data['count']=$count=array_key_exists('count', $data) ? $data['count'] : '';


        $pickingcommodity_data['status'] = array_key_exists('status', $data) ? $data['status'] : '';

        $pickingcommodity_data['store_id'] = $condition['store_id'] = current($userIdentity)['store_id'];
        ...
        $pickingcommodity_data['last_modified_by'] = current($userIdentity)['id'];
        $pickingcommodity_data['last_modified_time'] = date('Y-m-d H:i:s');

        if ($switch) {
            $pickingcommodity_data['created_time'] = date('Y-m-d H:i:s');
            $pickingcommodity_data['created_by'] = current($userIdentity)['id'];
        }


        return array_filter($pickingcommodity_data, function ($v) {
