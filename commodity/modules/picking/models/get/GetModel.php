...
namespace commodity\modules\picking\models\get;
...
class GetModel extends CommonModel {
    ...
    public function actionGetall() {
        ...
        $picking = $result->models;
        foreach ($picking as $key => $value) {

            $picking_condition['picking_id'] = $value['id'];
            $field = 'quantity,total_price';
            $picking_data = pickingcommodity_select::getProAll($picking_condition, $field);
            $quantity = 0;
            $total_price = 0;
            foreach ($picking_data as $value) {
                $quantity+=$value['quantity'];
                $total_price+=$value['total_price'];
            }
            $picking[$key]['quantity'] = $quantity;
            $picking[$key]['total_price'] = $total_price;
        }
